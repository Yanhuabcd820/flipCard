...
const view = {
  ...
  renderTimes(tries) {
    document.querySelector('.tried').textContent = `You've tried: ${tries} times`
  },
  appendWrongAnimation(...card) {
    card.map(card => {
      card.classList.add('wrong')
      card.addEventListener('animationend', event => {
        event.target.remove('wrong')
      },
        {
          once: true //addEventListener 觸發完就立刻消失
        })
    })
  },
